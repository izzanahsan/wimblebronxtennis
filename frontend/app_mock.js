...
async function loadAll() {
  ...
  const mockSeasons = [
    { id: 's1', name: 'Season 1', startDate: '2026-01-01', endDate: '2026-03-31', winner: 'Player 1', format: { type: 'firstto', n: 4 }, playerIds: ['p1', 'p2', 'p3', 'p4'] },
    { id: 's2', name: 'Season 2', startDate: '2026-04-01', endDate: '2026-06-30', winner: null, format: { type: 'firstto', n: 4 }, playerIds: ['p1', 'p2', 'p3', 'p4', 'p5'] }
  ];
  ...
  const mockMatches = [
    { id: 'm1', seasonId: 's2', date: today(), teamA: ['p1', 'p2'], teamB: ['p3', 'p4'], gamesA: 4, gamesB: 2, winner: 'A', format: { type: 'firstto', n: 4 } },
    { id: 'm2', seasonId: 's2', date: today(), teamA: ['p1', 'p3'], teamB: ['p2', 'p4'], gamesA: 1, gamesB: 4, winner: 'B', format: { type: 'firstto', n: 4 } }
  ];
  
  const mockLocked = [today()];
  ...
  state.seasons = mockSeasons;
  state.allPlayers = mockPlayers;
  
  state.seasonPlayers = {};
  mockSeasons.forEach(s => {
    state.seasonPlayers[s.id] = s.playerIds || [];
  });
  
  state.matches = mockMatches;
  state.lockedDates = mockLocked;
  state.availability = mockAvail;
...
async function confirmRemovePlayer() {
  ...
  renderPlayersPage(); renderStandings();
  toast(`✅ Player removed from season.`);
}
...
function setupLiveListener(seasonId) {
  window.addEventListener('storage', (e) => {
    if (e.key === LIVE_STORE) {
      ...
      if (d && d.seasonId === seasonId && d.selA && d.selB) {
        live = d.live;
        selA = d.selA; selB = d.selB;
        renderLiveWidget();
        if (scoreMode === 'live') renderLiveScore();
      } else if (!d) {
...
async function confirmMatch() {
  ...
  const payload = { id: `m${state.matches.length + 1}`, seasonId: state.currentSeason, date: today(), teamA: selA, teamB: selB, gamesA: scoreA, gamesB: scoreB, winner: w, format: fmt };
...
async function startNewSeason() {
  ...
  const data = { id: `s${state.seasons.length + 1}`, name: newName, startDate: startDate, endDate: endDate, format: modalFmt, playerIds: [...nsSelectedPlayers] };
...
async function uploadPlayerPhoto(playerId, file) {
  if (!file) return;
  toast('Photo upload simulated (mock).');
}
